...
local function _set_line_counts(self)
    ...
    self.num_visible_lines = math.min(self.num_visible_lines, #self.mTable)
    
    self.num_hidden_lines = math.max(0, #self.mTable - self.num_visible_lines)
...
local function _on_resize(self)
    ...
    local viewable_lines_pct = self.num_visible_lines / #self.mTable
...
local function _initialize_listview(self, width, height, left, top)
    ...
    control:SetHandler("OnResizeStop", function (self)
        ...
        _on_resize(me)
        me:update()  -- Bug fix. Resize doesn't fix what is currently listed.
    end)
...
function ListView:update()
    ...
    if Sliders.throttle(self, 0.05) then
        return
    end
    ...
    for i, line in pairs(self.control.lines) do
        local message = self.mTable[i + self.offset] -- self.offset = how much we've scrolled down
...
-- TODO Remove this crap. allow it to accept a table
function ListView:add_message(message)
    table.insert(self.mTable, " " .. message) -- poor man's padding. for now.
    _on_resize(self)
end

Sliders.ListView = ListView
